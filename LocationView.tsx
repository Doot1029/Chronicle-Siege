...
const LocationView: React.FC<LocationViewProps> = ({ locations, players, playerPositions, currentPlayerId, onMove }) => {
                    ...
                    {connectedLocations.length > 0 ? (
                        connectedLocations.map(loc => (
                            <button
                                ...
                                className="w-full text-left flex items-center justify-between gap-2 p-2 bg-background hover:bg-primary/20 rounded-lg transition-colors"
                            >
                                <span>{loc.name}</span>
